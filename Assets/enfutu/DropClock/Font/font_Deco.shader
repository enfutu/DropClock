    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom (triangle v2g input[3], inout TriangleStream<g2f> stream) 
			{
				...
				float isLeft = step(.0001, _UdonPlateValue[selectNum].w);       //プラス入ってるなら退室済み
				...
				float2 target1 = 0;
				//target1.x = (id - (characterNum - 1) * .5) * .025 * selectPower;
				...
				float _now = lerp(_UdonSelectValue.z, _UdonPlateValue[selectNum].w, isLeft);	//退室済みなら固定。BlitShaderに合わせてる。
				float platesDurationTime = _UdonPlateValue[selectNum].z;
				float a = .00002 * (_now - platesDurationTime); //.00001だと6時間で半分くらいだった
                a = min(1.5f, a);
            ...
            fixed4 frag (g2f i) : SV_Target
            {
				...
				//float2 st2 = i.uv2 + .5;
				//float rt = 1 - saturate(.08 - tex2D(_RT, st2).r) * 10;
				//rt = saturate(rt * 5);
